            ...
            <div class="carousel-inner">
                @foreach ($postJobsWithFirstService as $index => $postJob)
                    <div class="carousel-item {{ $index == 0 ? 'active' : '' }}">
                        <div class="d-block w-100" style="background-image: url('{{ asset('bg/' . $backgroundImages[$index % count($backgroundImages)]) }}'); height: 350px; background-size: cover; position: relative;">
                            <div class="carousel-caption d-none d-md-block" style="background: rgba(0, 0, 0, 0.5); padding: 40px; border-radius: 10px;">
                                <div class="row align-items-center">
                                    <div class="col-4">
                                        <img src="{{ asset('storage/logos/' . $postJob->employer->company->logo) }}" alt="Logo" class="img-fluid" style="width: 150px; height: auto; border-radius: 10%; background: #fff; padding: 5px;">
                                        <h4 class="text-white mt-2">{{ $postJob->employer->company->company_name }}</h4>
                                    </div>
                                    <div class="col-8 text-start">
                                        <h4 class="text-white">{{ $postJob->job_title }}</h4>
                                        <div class="text-white">{{ $postJob->job_description }}</div>
                                        <div class="d-flex mb-2">
                                            <div class='mt-1'><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-cash-coin" viewBox="0 0 16 16">
                                            ...
                                            <div class="text-white ml-2"> : {{ $postJob->salary }}</div>
                                        </div>
                                        <div class="d-flex mb-2">
                                            <div class="mt-1">
                                            ...
                                            <div><p> : {{ $postJob->employer->company->location }}</p></div>
                                        </div>

                                        <a href="{{ route('postjob.show', $postJob->id) }}" class="btn btn-primary" style="background-color: #007bff; border-color: #007bff; border-radius: 25px; padding: 10px 20px; font-size: 16px; font-weight: bold; text-transform: uppercase; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); transition: background-color 0.3s, transform 0.3s;">
                                            Ứng tuyển ngay
                                        </a>
                                    </div>
                                </div>
